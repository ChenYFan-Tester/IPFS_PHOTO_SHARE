...
// EXTERNAL MODULE: ./node_modules/_sjcl@1.0.8@sjcl/sjcl.js
...
async function ipfscloud(req) {
    ...
    switch (rp(path)) {
        case '/get':
            if (sq('en') === 'true') {
                return fetch(`https://ipfs.io/ipfs/${sjcl_default().decrypt(PASSKEY, atob(sq('hash')))}`)
            } else {
                return fetch(`https://ipfs.io/ipfs/${sq('hash')}`)
            }
        case '/custom':
            return src_gres({
                ctx: custom.replace(/<!--RECAP-->/g, RECAP)
                    .replace(/<!--RECAP_URL-->/g, (() => {
                        if (config_namespaceObject.Yp) {
        ...
        case '/upload':
            ...
            try {
                ...
                const res = await (await (fetch(config_namespaceObject.sx, req))).json()
                ...
                for (var i in config_namespaceObject.NC) {
                    if (res.Name.endsWith(config_namespaceObject.NC[i])) {
                        if (Number(res.Size) <= config_namespaceObject.YH) {
                            if (res.Name.length <= config_namespaceObject.n4) {
                                await KV.put('PHOTOSHARE', JSON.stringify(SHARELIST))
                                return src_gres({ type: "json", ctx: { code: 0, success: true } })
                            } else {
                                return src_gres({ type: "json", ctx: { code: -1, success: false,length:Number(res.Name.length),accept:config_namespaceObject.n4 }, msg: "这文件的名字有点长" })

                            }
                        } else {
                            return src_gres({ type: "json", ctx: { code: -1, success: false ,size:Number(res.Size),accept:config_namespaceObject.YH}, msg: "这文件太大了" })
                        }
                    }
                }
                return src_gres({ type: "json", ctx: { code: -1, success: false }, msg: "不合法的后缀" })
            } catch (t) {
                return src_gres({ type: "json", ctx: { code: -1, success: false }, msg: JSON.stringify(t) })
            }
        case '/':
            let piclist = []
                ...
                for (var i in SHARELIST) {
                    if (piclist.indexOf(SHARELIST[i].hash) === -1) {
                        y = init.replace(/<!--HASH-->/g, SHARELIST[i].hash)
                            .replace(/<!--URL-->/g, (() => {
                                if (config_namespaceObject.hK) {
                                    return `https://${domain}/get?hash=${btoa(sjcl_default().encrypt(PASSKEY, SHARELIST[i].hash))}&en=true`
                                } else {
            ...
            return src_gres({
                ctx: cjs_js_index.replace(/<ul class="cd-slider">/g, `<ul class="cd-slider unset">${hinit}`)
                ,
                type: "html"
            })
    }
